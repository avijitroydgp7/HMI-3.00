...
class TagDialog(QDialog):
    def __init__(self, parent=None, existing_tag_numbers=None, initial_data=None):
        ...
        if initial_data:
            ...
            # For new tag, set to next available number
    ...
    def _get_next_available_number(self):
        """Calculates and returns the next available tag number."""
        if not self.existing_tag_numbers:
            return 1
        # Return max number + 1
        return max(self.existing_tag_numbers) + 1
